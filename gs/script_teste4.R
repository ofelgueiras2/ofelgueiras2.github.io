...
# 🔹 Converter para data.frame e remover nomes das colunas
simulador <- as.data.frame(simulador)
names(simulador) <- NULL

# 🔸 Função para escrever CSV com BOM e sem cabeçalhos
escrever_csv_bom <- function(df, caminho) {
  temp <- tempfile()
  write.table(df,
              file = temp,
              sep = ";",
  ...
  conteudo <- readLines(temp, encoding = "UTF-8")
  bom <- "\ufeff"
  if (length(conteudo) > 0) {
    conteudo[1] <- paste0(bom, conteudo[1])
  }
  writeLines(conteudo, caminho, useBytes = TRUE)
  file.remove(temp)
}

# 🔹 Guardar ficheiro principal
escrever_csv_bom(simulador, "gs/SimuladorEletricidade_OF_MN_2025_3.csv")
cat("✅ Ficheiro principal guardado com BOM\n")

# 🔹 Recarregar para segurança
simulador <- read_delim("gs/SimuladorEletricidade_OF_MN_2025_3.csv",
                        delim = ";",
                        col_names = FALSE,
                        locale = locale(encoding = "UTF-8"),
                        col_types = cols(.default = col_character()))
...
# 🔹 Definir intervalos
col_ini_grande <- 48  # AV
col_fim_grande <- 55  # BC
col_fim_basico <- 60  # BH
linha_fim_basico <- 368

# 🔹 Simulador_grande.csv
simulador_grande <- simulador[, col_ini_grande:col_fim_grande]
escrever_csv_bom(simulador_grande, "gs/Simulador_grande.csv")

# 🔹 Simulador_basico.csv
simulador_basico <- simulador
simulador_basico[, col_ini_grande:col_fim_grande] <- ""
simulador_basico <- simulador_basico[1:linha_fim_basico, 1:col_fim_basico]
escrever_csv_bom(simulador_basico, "gs/Simulador_basico.csv")

cat("✅ Ficheiros guardados com BOM: Simulador_basico.csv e Simulador_grande.csv\n")
